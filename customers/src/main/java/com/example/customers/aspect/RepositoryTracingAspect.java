...
public class RepositoryTracingAspect {
     ...
     */
    @Around("execution(* com.example.customer.repository.*Repository.*(..))")
    public Object traceRepositoryMethods(ProceedingJoinPoint joinPoint) throws Throwable {
        String className = joinPoint.getTarget().getClass().getSimpleName();
        String methodName = joinPoint.getSignature().getName();
        String operationName = String.format("customers.repository.%s.%s", 
        ...
        BusinessContextTracer.BusinessContext context = businessContextTracer.createContext()
                .serviceCall("customer-repository", methodName)
    ...
    private void addEntityContext(BusinessContextTracer.BusinessContext context, Object firstArg, String methodName) {
        try {
            if (firstArg instanceof String && methodName.contains("Id")) {
                ...
                context.customerId((String) firstArg);
            } else if (firstArg != null && firstArg.getClass().getSimpleName().contains("Customer")) {
                // This is likely a Customer entity
                extractCustomerContext(context, firstArg);
            }
        } catch (Exception e) {
    ...
    /**
     * Extract context from Customer entity using reflection
     */
    private void extractCustomerContext(BusinessContextTracer.BusinessContext context, Object customer) {
        try {
            // Use reflection to get customer properties
            var customerClass = customer.getClass();
            var customerIdField = customerClass.getDeclaredField("customerId");

            customerIdField.setAccessible(true);

            String customerId = (String) customerIdField.get(customer);

            if (customerId != null) context.customerId(customerId);

        } catch (Exception e) {
            // Ignore reflection errors
            logger.debug("Could not extract customer context: {}", e.getMessage());
        }
    }
